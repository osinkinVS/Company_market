...
df = pd.read_csv('companies.csv', delimiter=';', encoding='utf-8-sig')
...
def get_checko_contacts(inn):
    ...
    try:
        ...
        if data.get('data'):
            ...
            if not employees:  # Если нет нового - ищем старое
...
for i, inn in enumerate(inn_list, 1):
    ...
    # Пауза 1 секунда (лимит 100 запросов/день)
    time.sleep(1)

# Сохраняем в CSV
if results:
    with open('checko_contacts.csv', 'w', newline='', encoding='utf-8-sig') as f:
        writer = csv.DictWriter(f, fieldnames=['name', 'inn', 'phone', 'email', 'web', 'employees'], delimiter=';')
        writer.writeheader()
